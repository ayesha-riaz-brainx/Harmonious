...
function normalizeTaskLabels(tasks) {
  return (Array.isArray(tasks) ? tasks : []).map((task) => {
    if (
      ...
      /^drink\s*water$/i.test(task.label.trim())
    ) {
      return {
        ...task,
        label: 'Drink water (habit)',
